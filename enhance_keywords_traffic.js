...
const advancedKeywordDatabase = {
  ...
  'video-compressor': {
    ...
    highVolume: ['video compressor online free', 'compress video file size', 'reduce video size'],
    longTail: [
  ...
  'password-generator': {
    primary: ['password generator', 'strong password generator', 'secure password'],
    highVolume: ['password generator online', 'random password generator', 'password creator'],
    longTail: [
  ...
  'word-counter': {
    ...
    highVolume: ['word count tool', 'character count online', 'word counter online'],
    longTail: [
  ...
  'image-compressor': {
    ...
    highVolume: ['compress image online', 'image size reducer', 'photo compressor'],
    longTail: [
  ...
  'youtube-downloader': {
    primary: ['youtube downloader', 'download youtube video', 'youtube video downloader'],
...
function getCategoryKeywords(category) {
  const categoryKeywords = {
    video: [
      'video editing online', 'video processing tool', 'video optimization',
      'video converter free', 'video editor professional', 'video tools online'
    ],
    image: [
      'image editing online', 'photo editor free', 'image optimization',
      'image converter tool', 'photo processing online', 'image tools professional'
    ],
    text: [
      'text processing tool', 'text editor online', 'text analysis free',
      'writing tools online', 'text formatter free', 'document processing'
    ],
    pdf: [
      'pdf editor online', 'pdf tools free', 'document converter',
      'pdf processing tool', 'pdf editor professional', 'document tools'
    ],
    developer: [
      'developer tools online', 'coding tools free', 'programming utilities',
      'web developer tools', 'code formatter online', 'development tools'
    ],
    calculator: [
      'calculator online free', 'math tools online', 'calculation utilities',
      'business calculator', 'scientific calculator', 'calculator tools'
    ]
  };
  
  return categoryKeywords[category] || ['online tool', 'free utility', 'web application'];
}
...
function getToolCategory(filePath) {
  ...
  if (filePath.includes('(youtube)') || filePath.includes('youtube-')) return 'youtube';
  if (filePath.includes('(audio)') || filePath.includes('audio-')) return 'audio';
  return 'general';
}
...
function enhanceKeywords(content, filePath) {
  ...
  keywords: [
    ...
    updatedContent = updatedContent.replace(/description:\s*["'][^"']*["']/, `$&${addKeywords}`);
  }
  
  // Enhance title if it's too short or generic
  const titleMatch = content.match(/title:\s*["'`]([^"'`]+)["'`]/);
  if (titleMatch && (titleMatch[1].length < 40 || titleMatch[1].includes('Free Online Tool'))) {
    updatedContent = updatedContent.replace(titleMatch[0], `title: "${optimized.title}"`);
  }
  
  // Enhance description if it's generic
  const descMatch = content.match(/description:\s*["'`]([^"'`]+)["'`]/);
  if (descMatch && descMatch[1].includes('Professional') && descMatch[1].includes('Free online processing')) {
...
function enhanceAllKeywords() {
  ...
  pageFiles.forEach((filePath, index) => {
    try {
      ...
      const keywordCount = keywordMatch ? (keywordMatch[0].match(/"/g) || []).length / 2 : 0;
      ...
      if (keywordCount < 15) {
        ...
        if (enhancedContent !== content) {
          ...
          console.log(`✅ Enhanced keywords: ${relativePath} (${keywordCount} → 30+ keywords)`);
          enhancedCount++;
          keywordIssuesFixed++;
        }
      } else {
        console.log(`⚪ Keywords OK: ${relativePath} (${keywordCount} keywords)`);
      }
  ...
  highTrafficTools.forEach(tool => {
    ...
    console.log(`🎯 ${tool}: ${data.monthlySearches.toLocaleString()} monthly searches`);
  });
  
  console.log(`\n🚀 TOTAL ADDRESSABLE SEARCH VOLUME: ${totalMonthlySearches.toLocaleString()} monthly searches`);
  console.log(`💰 Potential Monthly Visitors (5% capture rate): ${Math.round(totalMonthlySearches * 0.05).toLocaleString()}`);
  console.log(`🎊 Potential Monthly Visitors (15% capture rate): ${Math.round(totalMonthlySearches * 0.15).toLocaleString()}`);
}
