...
function findToolIds(dir, toolIds) {
...
// Now let's try to parse tools.json to verify tool IDs
const toolsJsonPath = 'src/constants/tools.json';
const toolsData = JSON.parse(fs.readFileSync(toolsJsonPath, 'utf8'));
const validToolIds = toolsData.map(t => t.id);

console.log(`Matched against tools.json, valid tools count: ${validToolIds.length}`);

// We need to simulate the RegisteredToolMount logic to see which hit GenericNotice.
// We can just read the RegisteredToolMount.tsx and see what's there.
const mountCode = fs.readFileSync('src/components/tools/shared/RegisteredToolMount.tsx', 'utf8');
const idRegistryCode = fs.readFileSync('src/components/tools/shared/tool-id-registry.ts', 'utf8');

fs.writeFileSync('scratch/simulate.ts', `
import { UNIT_TOOL_MAP, BASE_CONVERTER_MAP, IMAGE_FORMAT_TOOL_MAP } from '../src/components/tools/shared/tool-id-registry';

const toolIds = ${JSON.stringify(toolIds)};
