...
const seoTemplates = {
  video: {
    titleTemplate: (toolName) =>
      `${toolName} - Free Online Video Tool | High-Quality Results`,
    descriptionTemplate: (toolName, action) =>
      `Professional ${toolName.toLowerCase()} tool. ${action} videos online for free with high quality output. No watermarks, no registration required. Fast & secure processing.`,
    keywords: [
      "video tool",
      "online video",
      "free video",
  ...
  calculator: {
    titleTemplate: (toolName) =>
      `${toolName} - Free Online Calculator | Instant Results`,
    descriptionTemplate: (toolName, purpose) =>
      `Free ${toolName.toLowerCase()} with instant accurate results. ${purpose} online calculator for students, professionals & everyone. Easy to use, mobile-friendly interface.`,
    keywords: [
      "calculator",
      "online calculator",
      "free calculator",
  ...
  converter: {
    titleTemplate: (toolName) =>
      `${toolName} - Free Online Converter | Instant Conversion`,
    descriptionTemplate: (toolName, conversion) =>
      `Convert ${conversion} instantly with our free ${toolName.toLowerCase()}. High-quality conversion with privacy protection. No registration required, unlimited use.`,
    keywords: [
      "converter",
      "online converter",
      "free converter",
  ...
  generator: {
    titleTemplate: (toolName) =>
      `${toolName} - Free Online Generator | Create Instantly`,
    descriptionTemplate: (toolName, generates) =>
      `Generate ${generates} instantly with our free ${toolName.toLowerCase()}. Professional quality results, unlimited generations. Perfect for creators, developers & businesses.`,
    keywords: [
      "generator",
      "online generator",
      "free generator",
  ...
  text: {
    titleTemplate: (toolName) =>
      `${toolName} - Free Text Tool | Professional Text Processing`,
    descriptionTemplate: (toolName, process) =>
      `Professional ${toolName.toLowerCase()} to ${process} text instantly. Free online text tool with advanced features. Perfect for writers, students & professionals.`,
    keywords: [
      "text tool",
      "online text tool",
      "free text tool",
  ...
  image: {
    titleTemplate: (toolName) =>
      `${toolName} - Free Image Tool | Professional Quality`,
    descriptionTemplate: (toolName, action) =>
      `Professional ${toolName.toLowerCase()} for ${action} images online. Free, fast & secure image processing. No watermarks, maintains original quality.`,
    keywords: [
      "image tool",
      "online image tool",
      "free image tool",
  ...
  pdf: {
    titleTemplate: (toolName) =>
      `${toolName} - Free PDF Tool | Professional PDF Processing`,
    descriptionTemplate: (toolName, action) =>
      `Professional ${toolName.toLowerCase()} to ${action} PDF files online. Free, secure & unlimited PDF processing. No email required, instant results.`,
    keywords: [
      "pdf tool",
      "online pdf tool",
      "free pdf tool",
  ...
  seo: {
    titleTemplate: (toolName) =>
      `${toolName} - Free SEO Tool | Boost Your Rankings`,
    descriptionTemplate: (toolName, purpose) =>
      `Professional ${toolName.toLowerCase()} for ${purpose}. Free SEO analysis and optimization tool. Improve search rankings with data-driven insights.`,
    keywords: [
      "seo tool",
      "online seo tool",
      "free seo tool",
  ...
  developer: {
    titleTemplate: (toolName) =>
      `${toolName} - Free Developer Tool | Professional Coding`,
    descriptionTemplate: (toolName, purpose) =>
      `Professional ${toolName.toLowerCase()} for developers. ${purpose} with advanced features. Free coding tool for programmers, students & professionals.`,
    keywords: [
      "developer tool",
      "coding tool",
      "programming tool",
...
function generateOptimizedMetadata(filePath, currentContent) {
  ...
  let title =
    template?.titleTemplate?.(toolName) ||
    `${toolName} - Free Online Tool | Professional Results`;
  if (title.length > 65) {
  ...
  let description =
    template?.descriptionTemplate?.(toolName, "Process") ||
...
`;
    ...
    if (
      titleMatch &&
      (titleMatch[1].length < 30 || titleMatch[1].length > 65)
    ) {
...
`;
      ...
      updatedContent = updatedContent.replace(
        ...
        structuredDataCode + "export default function",
      );
...
function fixAllPages() {
  ...
  console.log(
    `📈 Success Rate: ${(((pageFiles.length - errorCount) / pageFiles.length) * 100).toFixed(1)}%`,
  );
}
