...
function extractMetadata(content, filePath) {
  ...
  metadata.hasStructuredData =
    content.includes("application/ld+json") || content.includes("schema.org");
  ...
  if (!metadata.hasTitle) {
    ...
    metadata.issues.push(
      `⚠️ Title length ${metadata.titleLength} chars (optimal: 30-65)`,
    );
  }
  ...
  if (!metadata.hasDescription) {
    ...
    metadata.descriptionLength < 120 ||
    metadata.descriptionLength > 160
  ) {
    metadata.issues.push(
      `⚠️ Description length ${metadata.descriptionLength} chars (optimal: 120-160)`,
    );
  }
  ...
  if (!metadata.hasKeywords) {
    ...
    metadata.issues.push(
      `⚠️ Only ${metadata.keywordCount} keywords (recommended: 15-25)`,
    );
  }
...
function analyzeSEO() {
    ...
    try {
      ...
      if (metadata.issues.length > 0) {
        ...
        if (metadata.description) {
          console.log(
            `   Desc: "${metadata.description.substring(0, 80)}..." (${metadata.descriptionLength} chars)`,
          );
        }
  ...
  console.log(
    `📈 Success Rate: ${(((pageFiles.length - pagesWithIssues) / pageFiles.length) * 100).toFixed(1)}%`,
  );
  ...
  pageAnalysis.forEach((page) => {
    page.metadata.issues.forEach((issue) => {
      ...
      else if (issue.includes("Missing description"))
        issueTypes["Missing description"]++;
      else if (issue.includes("Description length"))
        issueTypes["Description length"]++;
      else if (issue.includes("Missing keywords"))
        issueTypes["Missing keywords"]++;
      else if (issue.includes("keywords")) issueTypes["Keywords count"]++;
      else if (issue.includes("Missing OpenGraph"))
        issueTypes["Missing OpenGraph"]++;
      else if (issue.includes("Missing Twitter Card"))
        issueTypes["Missing Twitter Card"]++;
      else if (issue.includes("Missing canonical"))
        issueTypes["Missing canonical"]++;
      else if (issue.includes("Missing structured data"))
        issueTypes["Missing structured data"]++;
    });
  });
  ...
  priorityPages.forEach((page, index) => {
    console.log(
      `${index + 1}. ${page.path} (${page.metadata.issues.length} issues)`,
    );
  });
  ...
  const report = {
    summary: {
      ...
      totalIssues,
      successRate: (
        ((pageFiles.length - pagesWithIssues) / pageFiles.length) *
        100
      ).toFixed(1),
    },
...
console.log("\n🎯 NEXT STEPS:");
console.log(
  "1. Run: node scripts/fix_seo_issues.js - to automatically fix common issues",
);
