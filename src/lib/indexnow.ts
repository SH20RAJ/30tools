 ...
 * @param toolsData - Tools data from tools.json
 * @returns Success status
 */
export async function submitAllToolsToIndexNow(toolsData: any): Promise<boolean> {
	try {
		const urls: string[] = [];
 ...
 * @param slug - Tool slug or page path
 */
export async function notifyIndexNowOnUpdate(slug: string) {
