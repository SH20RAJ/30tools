...
const mainCategories = [
  'image', 'pdf', 'video', 'audio', 'text', 'code', 
  'seo', 'design', 'utilities', 'developer', 'youtube'
];
...
export async function getOtherTools() {
  try {
    ...
    const otherTools = allTools.filter(tool => 
      !mainCategories.includes(tool.category)
    );
...
export async function getAllCategories() {
  try {
    const categories = Object.entries(toolsData.categories).map(([key, category]) => ({
      slug: key,
      name: category.name,
