...
export function generateOrganizationSchema() {
  return {
    ...
    "description": "Professional online toolkit with 135+ free tools for image processing, PDF manipulation, video conversion, text analysis, and more. No registration required, privacy-focused.",
    "foundingDate": "2024",
    "sameAs": [
...
export function generateWebApplicationSchema() {
  return {
    ...
    "name": "30tools - Free Online Toolkit",
    "url": "https://30tools.com",
    "description": "Professional online toolkit with 135+ free tools for image processing, PDF manipulation, video conversion, text analysis, and more.",
...
export function generateSemanticKeywords(category) {
  const semanticMap = {
    image: ['photo', 'picture', 'graphics', 'visual', 'imagery', 'bitmap', 'raster'],
    ...
    seo: ['search engine optimization', 'ranking', 'visibility', 'traffic', 'organic'],
...
export function generateInternalLinks(currentTool) {
  ...
  const complementaryTools = allTools.filter(tool => {
    if (currentTool.category === 'image' && ['pdf', 'video'].includes(tool.category)) return true;
    if (currentTool.category === 'pdf' && ['image', 'text'].includes(tool.category)) return true;
    if (currentTool.category === 'video' && ['image', 'audio'].includes(tool.category)) return true;
    return false;
  }).slice(0, 3);
