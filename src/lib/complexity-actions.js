 ...
 */
export async function analyzeComplexity(code, language = 'javascript', model = 'qwen-coder') {
  try {
...
7. Respond ONLY with the JSON object, no additional text`;
    ...
    if (!analysisResult.timeComplexity || !analysisResult.spaceComplexity) {
      ...
      throw new Error('AI response missing required complexity analysis fields');
    }
...
export async function getAvailableModels() {
  try {
    ...
    const codeAnalysisModels = models.filter(model => 
    ...
    console.log(`✅ Found ${codeAnalysisModels.length} suitable models for code analysis`);
