...
export async function getToolSEOContent(toolName, category) {
  try {
    ...
    const content = {
      ...
      description: `Free ${toolName} online tool. ${keywords.slice(0, 3).map(k => k.keyword).join(', ')}. No registration required, works on all devices.`,
...
function generateFAQs(toolName, category, keywords) {
  const questionKeywords = keywords.filter(k => k.tag === 'questions').slice(0, 5);
...
function getToolDescription(toolName, category) {
  const descriptions = {
    'video downloader': 'download videos from social media platforms without watermarks',
...
function getUsageSteps(toolName) {
  const steps = {
    ...
    'pdf merger': 'upload multiple PDF files, arrange them in order, and merge them into one PDF',
    'keyword research': 'enter your main keyword, select search engine, and get keyword suggestions',
    'text editor': 'paste or type your text, apply formatting, and export the result',
    'color picker': 'click on any color or enter hex codes to get color information',
    'qr generator': 'enter the text or URL, customize settings, and generate your QR code',
    'url shortener': 'paste your long URL, optionally add custom alias, and get a short link'
  };
...
function getPreparationStep(toolName) {
  ...
  return preparations[toolName.toLowerCase()] || 'Prepare your input according to the tool requirements';
}
  ...
  return {
    ...
    howToGuide: {
      ...
      steps: [
        ...
        { title: 'Input your data', description: 'Enter the required information' },
