...
export async function checkEmailBreaches(email: string) {
	try {
		...
		if (!response.ok) {
			...
			return {
				success: true,
				data: { breaches: [] as BreachData[], pastes: [] as any[] },
				email: email,
			};
		}
		...
		const breaches: BreachData[] = data.result
			? data.result.map((breach: any) => ({
...
export async function checkPasswordBreaches(passwordHash: string) {
	try {
