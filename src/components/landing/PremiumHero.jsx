	...
	return (
		<div className="relative pt-20 pb-16 md:pt-32 md:pb-24 overflow-hidden">
			...
			<div className="max-w-4xl mx-auto text-center px-4">
				{/* Badge */}
				<div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-semibold mb-8 animate-in">
				...
				>
					<div className="relative flex items-center">
						...
						<Input
							...
							className="h-16 pl-16 pr-32 rounded-2xl bg-card border-border/50 text-lg shadow-2xl focus-visible:ring-primary/20 transition-all placeholder:text-muted-foreground/60"
							value={query}
							onChange={(e) => setQuery(e.target.value)}
						/>
						<Button 
							type="submit"
							className="absolute right-2 h-12 px-6 rounded-xl bg-primary hover:bg-primary/90 text-white font-medium"
						>
							Find Tool
						</Button>
					</div>
				...
				<div className="flex flex-wrap items-center justify-center gap-3 mb-16 animate-in" style={{ animationDelay: '200ms' }}>
					...
					{trendingTools.map((tool) => (
						<a
							key={tool.name}
							href={tool.href}
							className="px-4 py-1.5 rounded-full bg-secondary/50 border border-border hover:border-primary/30 hover:bg-secondary transition-all text-sm font-medium"
						>
							{tool.name}
						</a>
					))}
