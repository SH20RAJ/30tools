...
import { Copy, Download, Code, Building2, Package, FileText, Star, Calendar, HelpCircle } from 'lucide-react';
...
export default function SchemaGenerator() {
  ...
  const schemaFields = {
    LocalBusiness: [
      ...
      { key: 'addressRegion', label: 'State/Region', type: 'text', required: false },
      { key: 'postalCode', label: 'Postal Code', type: 'text', required: false },
      { key: 'addressCountry', label: 'Country', type: 'text', required: false },
      { key: 'priceRange', label: 'Price Range (e.g., $$)', type: 'text', required: false },
      { key: 'openingHours', label: 'Opening Hours (e.g., Mo-Fr 09:00-17:00)', type: 'text', required: false }
    ],
    Product: [
      ...
      { key: 'currency', label: 'Currency (e.g., USD)', type: 'text', required: false },
      { key: 'availability', label: 'Availability', type: 'select', options: ['InStock', 'OutOfStock', 'PreOrder'], required: false },
      { key: 'ratingValue', label: 'Rating (1-5)', type: 'number', required: false },
      { key: 'reviewCount', label: 'Review Count', type: 'number', required: false }
    ],
    Article: [
      ...
      { key: 'datePublished', label: 'Publish Date (YYYY-MM-DD)', type: 'date', required: true },
      { key: 'dateModified', label: 'Modified Date (YYYY-MM-DD)', type: 'date', required: false },
      { key: 'url', label: 'Article URL', type: 'url', required: true }
    ],
    Review: [
      { key: 'reviewBody', label: 'Review Text', type: 'textarea', required: true },
      { key: 'ratingValue', label: 'Rating (1-5)', type: 'number', required: true },
      { key: 'bestRating', label: 'Best Rating (usually 5)', type: 'number', required: false },
      { key: 'worstRating', label: 'Worst Rating (usually 1)', type: 'number', required: false },
      { key: 'author', label: 'Reviewer Name', type: 'text', required: true },
      { key: 'datePublished', label: 'Review Date (YYYY-MM-DD)', type: 'date', required: false },
      { key: 'itemName', label: 'Item Being Reviewed', type: 'text', required: true }
    ],
    Event: [
      { key: 'name', label: 'Event Name', type: 'text', required: true },
      { key: 'description', label: 'Description', type: 'textarea', required: true },
      { key: 'startDate', label: 'Start Date (YYYY-MM-DDTHH:MM)', type: 'datetime-local', required: true },
      { key: 'endDate', label: 'End Date (YYYY-MM-DDTHH:MM)', type: 'datetime-local', required: false },
      { key: 'locationName', label: 'Venue Name', type: 'text', required: false },
      { key: 'streetAddress', label: 'Street Address', type: 'text', required: false },
      { key: 'addressLocality', label: 'City', type: 'text', required: false },
      { key: 'addressRegion', label: 'State/Region', type: 'text', required: false },
      { key: 'postalCode', label: 'Postal Code', type: 'text', required: false },
    ...
    FAQ: [
      { key: 'questions', label: 'FAQ Questions & Answers (JSON format)', type: 'textarea', required: true, placeholder: '[{"question": "What is your return policy?", "answer": "We offer 30-day returns on all items."}]' }
    ]
  };
    ...
    if (schemaType === 'LocalBusiness') {
      ...
      if (formData.price && formData.currency) {
        schema.offers = {
          ...
          ...(formData.availability && { availability: `https://schema.org/${formData.availability}` })
        };
      }
  ...
  const selectedSchemaType = schemaTypes.find(type => type.value === schemaType);
            ...
            {selectedSchemaType && (
              <div className="space-y-4">
                <div className="flex items-center gap-2 mb-4">
                  <selectedSchemaType.icon className="w-5 h-5 text-green-500" />
                  <h3 className="font-semibold">{selectedSchemaType.label} Configuration</h3>
                </div>
            ...
            {generatedSchema ? (
              <div className="space-y-4">
                ...
                <div className="bg-green-50 dark:bg-green-950/20 border border-green-200 rounded-lg p-4">
                  <h4 className="font-medium text-green-800 dark:text-green-200 mb-2">Implementation Instructions:</h4>
                  <ol className="text-sm text-green-700 dark:text-green-300 space-y-1 list-decimal list-inside">
                    ...
                    <li>Paste it in your HTML &lt;head&gt; section or before &lt;/body&gt;</li>
                    <li>Wrap it in &lt;script type="application/ld+json"&gt;&lt;/script&gt; tags</li>
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p>Configure your schema type and fill in the details to generate markup</p>
              </div>
            )}
          </CardContent>
        </Card>
