	...
	const testSchema = () => {
		try {
			...
			if (hasContext && hasType) {
				setResult({
					valid: true,
					msg: "Valid JSON-LD Schema Structure detected.",
				});
			} else {
				setResult({
					valid: false,
			...
			{result && (
				...
				>
					<CardContent className="p-6 flex items-start gap-3">
						...
						<div>
							<h3
								className={`font-bold text-lg mb-1 ${result.valid ? "text-green-800 dark:text-green-300" : "text-red-800 dark:text-red-300"}`}
							>
