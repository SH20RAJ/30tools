					...
					data: [
						{
							label: "Title Tag",
							status: "good",
							value: `${domain} - Official Website`,
						},
						{
							label: "Meta Description",
							status: "warning",
							value: "Missing meta description or too short.",
						},
				...
				<Card className="animate-in fade-in slide-in-from-bottom-4 duration-500">
					<CardHeader className="border-b">
						<div className="flex items-center justify-between">
							...
							{result.score > 0 && (
								<div
									className={`text-2xl font-bold ${result.score > 80 ? "text-green-500" : result.score > 50 ? "text-amber-500" : "text-red-500"}`}
								>
									{result.score}/100
								</div>
							)}
					...
					<CardContent className="p-0">
						<div className="divide-y">
							{result.data.map((item, i) => (
								<div
									key={i}
									className="p-4 md:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-2"
								>
