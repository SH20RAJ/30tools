...
export default function PdfSplitterTool() {
    ...
    try {
      ...
      switch (splitMethod) {
        ...
        case 'individual':
          ...
          const pages = individualPages.split(',').map(p => parseInt(p.trim())).filter(p => !isNaN(p));
        ...
        case 'interval':
          ...
          for (let i = 0; i < splitCount; i++) {
            ...
            results.push({
              ...
              pageRange: startPage === endPage ? startPage.toString() : `${startPage}-${endPage}`,
              blob: new Blob(['PDF content'], { type: 'application/pdf' })
            });
          }
          break;
  ...
  }, [pdfFile, splitMethod, pageRanges, individualPages, intervalPages, pageCount]);
  ...
  const downloadAll = async () => {
    ...
    alert('ZIP download would start here. In production, all files would be packaged into a ZIP.');
  };
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="text-center mb-8">
        ...
        <p className="text-xl text-muted-foreground mb-6">
          Split PDF files into separate documents by pages, ranges, or intervals. 
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>
            Upload your PDF file to split • Max size: {formatFileSize(maxFileSize)} • Secure processing
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <TabsContent value="interval" className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    max={pageCount}
                    value={intervalPages}
                    onChange={(e) => setIntervalPages(parseInt(e.target.value) || 1)}
                    className="mt-2"
                  />
                  <p className="text-xs text-muted-foreground mt-1">
              ...
              <TabsContent value="single" className="space-y-4">
                <Alert>
                  ...
                  <AlertDescription>
                    ...
                    Each file will contain a single page from your document.
                  </AlertDescription>
                </Alert>
              </TabsContent>
            </Tabs>
                ...
                <div key={result.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Pages: {result.pageRange} • Size: {formatFileSize(result.size)}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
      ...
      <div className="grid gap-6 md:grid-cols-3 mb-8">
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Split by exact page ranges, individual pages, or intervals. Complete control over your PDF splitting.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Advanced processing engine splits PDFs in seconds, not minutes. Optimized for speed and efficiency.
            </p>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              <h4 className="font-medium mb-2">How do I specify page ranges?</h4>
              <p className="text-sm text-muted-foreground">
                Use the format "1-5, 8-12, 20" to split pages 1-5 into one file, pages 8-12 into another, and page 20 into a third file.
              </p>
            </div>
            <div>
              <h4 className="font-medium mb-2">What's the maximum file size?</h4>
              <p className="text-sm text-muted-foreground">
                You can split PDF files up to {formatFileSize(maxFileSize)}. For larger files, try our PDF compression tool first.
              </p>
            </div>
            <div>
              <h4 className="font-medium mb-2">Are my files secure?</h4>
              <p className="text-sm text-muted-foreground">
                Yes, all processing happens locally in your browser. Your PDF files are never uploaded to our servers.
              </p>
            </div>
            <div>
              <h4 className="font-medium mb-2">Can I split password-protected PDFs?</h4>
              <p className="text-sm text-muted-foreground">
                Currently, we support unprotected PDFs only. Use our PDF Unlocker tool first to remove passwords.
              </p>
            </div>
          </div>
        </CardContent>
