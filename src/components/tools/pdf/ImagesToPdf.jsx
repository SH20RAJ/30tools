		...
		try {
			...
			for (const img of images) {
				...
				try {
					...
					toast.error(
						`Could not embed ${img.file.name}. It might be a format incompatible with standard PDF embedding.`,
					);
					continue;
				}
				...
				{images.length > 0 && (
					<div className="space-y-4">
						<Label className="text-base font-medium">
							2. Organize Images ({images.length})
						</Label>
							...
							{images.map((img, index) => (
								...
								>
									...
									<button
										...
										className="absolute top-1 right-1 bg-red-500 text-white transition-opacity"
									>
										<X className="w-3 h-3" />
									</button>
