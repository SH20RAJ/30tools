...
export default function ImageConverterTool() {
  ...
  const supportedFormats = {
    ...
    output: {
      'jpeg': { ext: '.jpg', name: 'JPEG', icon: '📷', description: 'Best for photos, smaller file size' },
      'png': { ext: '.png', name: 'PNG', icon: '🖼️', description: 'Best for graphics, supports transparency' },
      'webp': { ext: '.webp', name: 'WebP', icon: '🌐', description: 'Modern format, excellent compression' },
      'gif': { ext: '.gif', name: 'GIF', icon: '🎞️', description: 'Supports animation, limited colors' },
      'bmp': { ext: '.bmp', name: 'BMP', icon: '🎨', description: 'Uncompressed, large file size' },
      'tiff': { ext: '.tiff', name: 'TIFF', icon: '📋', description: 'High quality, supports layers' }
    }
  };
  ...
  const getFormatIcon = (mimeType) => {
    const format = Object.keys(supportedFormats.input).find(key => mimeType.includes(key.split('/')[1]));
  ...
  const processFiles = (fileList) => {
    ...
    const newFiles = validFiles.map((file, index) => ({
      ...
      size: file.size,
      type: file.type,
      originalFormat: file.type.split('/')[1] || file.name.split('.').pop().toLowerCase(),
      status: 'ready',
      convertedBlob: null,
      convertedSize: null,
      preview: null
  ...
  const convertImage = async (fileData) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        const mimeType = conversionSettings.outputFormat === 'jpeg' ? 'image/jpeg' : `image/${conversionSettings.outputFormat}`;
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8 max-w-7xl">
                          ...
                          {Object.entries(supportedFormats.output).map(([key, format]) => (
                            <SelectItem key={key} value={key}>
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <div className="font-medium">{format.name} ({format.ext})</div>
                  ...
                  <TabsContent value="quality" className="space-y-4">
                    <div>
                      <Label className="flex justify-between">
                        <span>Quality</span>
                        <span className="text-sm font-mono">{conversionSettings.quality[0]}%</span>
                      </Label>
                      <Slider
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {[
                        ...
                        { value: 100, label: 'Maximum', desc: 'No compression' }
                      ].map((preset) => (
                        <Button
                          key={preset.value}
            ...
            {files.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Converting to {supportedFormats.output[conversionSettings.outputFormat].name} format
                  </CardDescription>
                </CardHeader>
                <CardContent>
                          ...
                          <div className="flex-shrink-0">
                            {fileData.preview ? (
                              ...
                              <div className="w-16 h-16 bg-muted rounded flex items-center justify-center">
                                <span className="text-2xl">{getFormatIcon(fileData.type)}</span>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-4 text-sm text-muted-foreground">
                              ...
                              <span>
                                ...
                                {fileData.convertedSize && ` (${formatFileSize(fileData.convertedSize)})`}
                              </span>
                              {savings && (
                                <Badge variant={savings.isSmaller ? "success" : "secondary"} className="text-xs">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    <span>Quality:</span>
                    <span className="font-medium">{conversionSettings.quality[0]}%</span>
                  </div>
                  {conversionSettings.enableResize && (
                    <div className="flex justify-between text-sm">
                      <span>Resize:</span>
                      <span className="font-medium">
                        {conversionSettings.resizeWidth || 'Auto'} × {conversionSettings.resizeHeight || 'Auto'}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <CardContent>
                <div className="space-y-3 text-sm">
                  <div>
                    ...
                    <div className="text-muted-foreground">Best for photos, smaller files</div>
                  </div>
                  <div>
                    <div className="font-medium">🖼️ PNG</div>
                    <div className="text-muted-foreground">Supports transparency, lossless</div>
                  </div>
                  <div>
                    <div className="font-medium">🌐 WebP</div>
                    <div className="text-muted-foreground">Modern, excellent compression</div>
                  </div>
                  <div>
                    <div className="font-medium">🎞️ GIF</div>
                    <div className="text-muted-foreground">Animations, limited colors</div>
                  </div>
                  <div>
                    <div className="font-medium">📱 HEIC</div>
                    <div className="text-muted-foreground">iPhone photos, efficient</div>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="mt-16 space-y-12">
          {/* How to Use */}
          <section>
            <h2 className="text-2xl font-bold mb-6">How to Convert Images Online</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card>
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Drag and drop your images or click to browse. Supports JPG, PNG, WebP, GIF, BMP, TIFF, and HEIC formats.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Select output format, adjust quality, and optionally resize images. Preview settings in real-time.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Convert and download individual files or get all converted images in a zip file.
                  </p>
                </CardContent>
              </Card>
            </div>
          ...
          {/* Common Conversions */}
          <section>
            <h2 className="text-2xl font-bold mb-6">Popular Image Conversions</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                { from: 'HEIC', to: 'JPG', desc: 'iPhone photos to universal format' },
                { from: 'PNG', to: 'JPG', desc: 'Reduce file size for web use' },
                { from: 'JPG', to: 'PNG', desc: 'Add transparency support' },
                { from: 'Any', to: 'WebP', desc: 'Modern web-optimized format' },
                { from: 'GIF', to: 'MP4', desc: 'Better compression for animations' },
                { from: 'BMP', to: 'JPG', desc: 'Compress uncompressed images' }
                ...
                <Card key={index}>
                  <CardContent className="p-4">
                    ...
                    <p className="text-sm text-muted-foreground">{conversion.desc}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
          ...
          {/* Use Cases */}
          <section>
            <h2 className="text-2xl font-bold mb-6">When to Use Image Converter</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <h3 className="text-xl font-semibold mb-4">Professional Use Cases</h3>
                <ul className="space-y-2 text-muted-foreground">
                  <li>• <strong>Web Development:</strong> Convert images to WebP for faster loading</li>
                  <li>• <strong>Print Design:</strong> Convert to TIFF for high-quality printing</li>
                  <li>• <strong>Social Media:</strong> Optimize images for different platforms</li>
                  <li>• <strong>Email Marketing:</strong> Reduce file sizes for newsletters</li>
                  <li>• <strong>E-commerce:</strong> Standardize product image formats</li>
                  <li>• <strong>Mobile Apps:</strong> Convert to appropriate formats for iOS/Android</li>
                </ul>
              </div>
              <div>
                <h3 className="text-xl font-semibold mb-4">Personal Use Cases</h3>
                <ul className="space-y-2 text-muted-foreground">
                  <li>• <strong>iPhone Photos:</strong> Convert HEIC to JPG for sharing</li>
                  <li>• <strong>Social Sharing:</strong> Optimize photos for Instagram, Facebook</li>
                  <li>• <strong>Website Building:</strong> Prepare images for your blog or portfolio</li>
                  <li>• <strong>Digital Storage:</strong> Reduce file sizes to save space</li>
                  <li>• <strong>Photo Editing:</strong> Convert to PNG for transparency effects</li>
                  <li>• <strong>Presentations:</strong> Optimize images for PowerPoint or Google Slides</li>
                </ul>
              </div>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card>
                <CardContent className="p-6">
                  ...
                  <p className="text-sm text-muted-foreground">
                    We support 8+ formats including JPG, PNG, WebP, GIF, BMP, TIFF, HEIC, and HEIF. Convert to any of the 6 most popular output formats.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="font-semibold mb-2">Is image conversion free?</h3>
                  <p className="text-sm text-muted-foreground">
                    Yes! Our image converter is completely free with no limits on file size, number of conversions, or watermarks on output files.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6">
                  ...
                  <p className="text-sm text-muted-foreground">
                    No! All image conversion happens in your browser. Your images never leave your device, ensuring complete privacy and security.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="font-semibold mb-2">Can I convert HEIC files from iPhone?</h3>
                  <p className="text-sm text-muted-foreground">
                    Absolutely! Our converter fully supports HEIC files from iPhone and can convert them to JPG, PNG, or any other supported format.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="font-semibold mb-2">Can I batch convert multiple images?</h3>
                  <p className="text-sm text-muted-foreground">
                    Yes! Upload multiple images at once and convert them all with the same settings. Download individually or as a zip file.
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="font-semibold mb-2">How do I choose the right quality setting?</h3>
                  <p className="text-sm text-muted-foreground">
                    For web use, 75-85% is ideal. For print, use 90-100%. For email attachments, 60-70% provides good balance of quality and file size.
                  </p>
                </CardContent>
              </Card>
            </div>
          ...
          <section className="bg-muted/30 p-8 rounded-lg">
            <h2 className="text-2xl font-bold mb-4">Professional Image Converter - Free & No Watermarks</h2>
            <div className="prose prose-sm max-w-none text-muted-foreground">
              <p className="mb-4">
                <strong>Convert images online for free</strong> with our professional image converter. Transform between JPG, PNG, WebP, GIF, BMP, TIFF, HEIC, and HEIF formats with advanced quality control and batch processing. Perfect for web developers, designers, photographers, and anyone who needs reliable image format conversion.
              </p>
              <p className="mb-4">
                <strong>Key features include:</strong> 8+ supported input formats, 6 optimized output formats, batch conversion capabilities, quality and compression control, image resizing options, EXIF data preservation, client-side processing for privacy, and professional-grade results with no watermarks.
              </p>
              <p>
                <strong>Popular conversions:</strong> HEIC to JPG for iPhone photos, PNG to JPG for web optimization, any format to WebP for modern web standards, GIF to MP4 for better video compression, and BMP to JPG for file size reduction. Our image converter handles all your format conversion needs with professional quality and complete privacy.
              </p>
            </div>
          </section>
        </div>
