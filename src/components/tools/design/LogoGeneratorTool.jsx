...
const fonts = [
  ...
  { name: 'Source Code Pro', value: 'Source Code Pro, monospace', category: 'Tech' }
];

const colorPalettes = [
  { name: 'Professional Blue', colors: ['#2563eb', '#1d4ed8', '#1e40af', '#3b82f6'] },
  { name: 'Modern Purple', colors: ['#7c3aed', '#8b5cf6', '#a855f7', '#c084fc'] },
  { name: 'Energy Orange', colors: ['#ea580c', '#f97316', '#fb923c', '#fed7aa'] },
  { name: 'Growth Green', colors: ['#059669', '#10b981', '#34d399', '#86efac'] },
  { name: 'Trust Blue', colors: ['#0ea5e9', '#06b6d4', '#22d3ee', '#67e8f9'] },
  { name: 'Creative Pink', colors: ['#db2777', '#ec4899', '#f472b6', '#fbcfe8'] },
  { name: 'Bold Red', colors: ['#dc2626', '#ef4444', '#f87171', '#fca5a5'] },
  { name: 'Minimal Gray', colors: ['#374151', '#6b7280', '#9ca3af', '#d1d5db'] }
];
...
export default function LogoGeneratorTool() {
  ...
  useEffect(() => {
    generateLogo();
  }, [logoText, selectedFont, fontSize, letterSpacing, selectedColor, selectedStyle, backgroundColor, isTransparent]);
  ...
  const drawTextWithSpacing = (ctx, text, x, y) => {
    ...
    const totalWidth = chars.reduce((acc, char) => acc + ctx.measureText(char).width, 0) + (chars.length - 1) * letterSpacing[0];
  ...
  const drawStyledText = (ctx, text, x, y) => {
    ...
    if (selectedStyle.gradient) {
      const gradient = ctx.createLinearGradient(x - 100, y - 25, x + 100, y + 25);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Create professional logos instantly with our free logo generator. Choose from text-based designs, custom fonts, colors, and effects. No design skills required.
          </p>
        </div>
            ...
            <Card className="card-cute">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Select value={selectedFont.name} onValueChange={(value) => {
                    ...
                    {logoStyles.map((style) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-muted-foreground mt-1">
                          ...
                          {style.shadow && 'Shadow • '}
                          {style.border && 'Border • '}
                          {!style.gradient && !style.shadow && !style.border && 'Clean & Simple'}
                        </div>
                      </button>
                    ))}
                  </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="card-cute">
              <CardHeader>
                ...
                <CardDescription>
                  Download your logo in high quality formats for web and print use
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-wrap gap-3">
                  <Button onClick={() => downloadLogo('png')} className="btn-cute">
                    <Download className="w-4 h-4 mr-2" />
                    Download PNG
                  </Button>
                  <Button onClick={() => downloadLogo('jpeg')} variant="outline">
                    <Download className="w-4 h-4 mr-2" />
                    Download JPEG
                  </Button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Create logos in seconds with real-time preview</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <div className="w-8 h-8 bg-muted dark:bg-primary/30 rounded-lg flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">15+ carefully selected fonts for any brand</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <div className="w-8 h-8 bg-muted dark:bg-primary/30 rounded-lg flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Curated color schemes for different industries</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <div className="w-8 h-8 bg-muted dark:bg-primary/30 rounded-lg flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Download as PNG, JPEG, or copy to clipboard</p>
                    </div>
                  </div>
                </div>
              </CardContent>
