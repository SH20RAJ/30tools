...
export default function FacebookDownloader() {
	...
	const handleDownload = async () => {
		...
		try {
			...
			if (!data || data.error) {
				throw new Error(
					"Could not find video. Make sure the link is public and correct.",
				);
			}
			...
			console.error(err);
			setError(
				"Failed to process the Facebook video. Please make sure the link is public and try again.",
			);
		} finally {
			setIsLoading(false);
		}
						...
						<div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
												...
												{videoData.medias &&
													videoData.medias.map((media, index) => (
														<Button
															key={index}
															variant={
																media.quality === "HD" ? "default" : "secondary"
															}
							...
							<div className="text-center">
								<p className="text-sm text-muted-foreground/60 italic">
									Clicking download will open the video in a new tab. Long press
									or right click to "Save Video As".
								</p>
							</div>
						</div>
					)}
