...
import { Search, Loader2, AlertCircle, CheckCircle2, Music, Download } from "lucide-react";
      ...
      {videoData && (
        <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="flex items-center gap-2 p-4 bg-green-500/10 border border-green-500/20 rounded-xl text-green-600">
            ...
            <span className="text-sm font-bold">{videoData.platform} video ready for download!</span>
          </div>
          ...
          <Card className="overflow-hidden border-border/50">
            <CardContent className="p-0 flex flex-col md:flex-row">
              ...
              <div className="p-6 flex-1 min-w-0">
                <h3 className="font-bold text-xl truncate mb-2">{videoData.title}</h3>
                <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                   {videoData.author && <span>By: {videoData.author}</span>}
                   {videoData.duration && <span>Duration: {videoData.duration}</span>}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 gap-3">
            {videoData.qualities.map((quality, idx) => (
              <div key={idx} className="flex items-center justify-between p-4 bg-card border border-border/50 rounded-xl hover:bg-accent transition-colors">
                ...
                <Button onClick={() => triggerDownload(quality)} variant="secondary" size="sm">
