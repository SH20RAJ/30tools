    ...
    try {
        ...
        if (!isSupported) {
            return NextResponse.json({ 
                error: 'Unsupported video platform. Please try TikTok, Facebook, Instagram, Twitter/X, Vimeo, Dailymotion, Reddit, or Rumble.' 
            }, { status: 400 });
        }
        ...
        if (!response.ok) {
            return NextResponse.json({ error: 'Failed to fetch video data from the service' }, { status: 500 });
        }
...
function parseVideoResponse(responseText, originalUrl) {
    ...
    return {
        ...
        duration: '0:30',
        author: platform === 'TikTok' ? '@tiktoker' : platform === 'Twitter/X' ? '@username' : '',
